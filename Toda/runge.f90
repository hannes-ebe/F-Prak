program runge_kutta
    ...
    ! declare variables
    real(kind=ikind), dimension(1d3) :: Q, I, phi, Tl
    ...
    do ci = 1,steps
        t=dt*ci
        Tl(ci+1)=t
        call runge_kutta_step(t,dt,Q(ci),I(ci),phi(i),Q(ci+1),I(ci+1),phi(i+1))
    end do
    !Save results
...
subroutine runge_kutta_step(t,dt,Q,I,phi,Q_new,I_new,phi_new)
    ...
    Ik2=fI(t+ dt/2,Q+dt/2*Ik1,I+dt/2*Ik1,I+dt/2*Ik1,phi+dt/2*Ik1,U_pp,L,R,U_S,C0)
    Ik3=fI(t+ dt/2,Q+dt/2*Ik2,I+dt/2*Ik2,I+dt/2*Ik2,phi+dt/2*Ik2,U_pp,L,R,U_S,C0)
